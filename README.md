...
The downside of this method is it remaps the keys on user login instead of boot, but I don't really care.  
The benefit of this method is we no longer need to give the terminal app ```input monitoring``` permissions, instead we will grant the permission to the Automator app.
The .sh script that the Automator app is wrapping can be found [here](replace me).
...
- Download the ```Remap Mac Keys.app``` from this repository and move it to your application folder
